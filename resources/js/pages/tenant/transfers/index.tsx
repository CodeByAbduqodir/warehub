            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                {documents.data.length === 0 ? (
                    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed py-16 text-center">
                        ...
                        <p className="text-sm text-muted-foreground">Перемещений нет</p>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b bg-muted/50">
                                        ...
                                        <th className="px-4 py-3 text-left font-medium text-muted-foreground">Откуда → Куда</th>
                                    ...
                                    {documents.data.map((doc) => (
                                        <tr key={doc.id} className="border-b last:border-0 hover:bg-muted/30">
                                            ...
                                            <td className="px-4 py-3 text-muted-foreground">
                                                <span>{doc.from_warehouse.name}</span>
                                                <ArrowRightLeft className="mx-1.5 inline size-3" />
                                                <span>{doc.to_warehouse.name}</span>
                                            </td>
                            ...
                            <div className="flex items-center justify-between text-sm text-muted-foreground">
                                ...
                                <div className="flex gap-2">
                                    {documents.current_page > 1 && (
                                        <Button variant="outline" size="sm" onClick={() => router.get('/transfers', { page: documents.current_page - 1 })}>
                                            Назад
                                        </Button>
                                    )}
